...
import org.jcommons.common.ClassUtils;
import org.jcommons.common.ReflectUtils;
...
public class BeanXMLReader implements XMLReader {
		...
		if (ClassUtils.isPojo(clazz)) {
			...
			for (Field f : fields) {
				if (List.class.isAssignableFrom(f.getType())) {
					...
					Class<?> cls = ClassUtils.getPojoClass(type.toString()
							.replace("class ", ""));
	...
	private <T> T readRecursion(Element bean) throws Exception {
		...
		ReflectUtils ru = new ReflectUtils(o);
		...
		for (Field f : fields) {
			...
			} else if (ClassUtils.isPojo(f.getType())) {
			...
			} else if (ClassUtils.isListClass(f)) {
			...
			} else if (ClassUtils.isListString(f)) {
				List<?> eList = bean.elements(n);
				if (eList == null)
					continue;
